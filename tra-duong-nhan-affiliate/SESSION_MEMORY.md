...
- Product catalog index: `app/san-pham/page.tsx` (added 2026-07-27) — lists all 6 products via `ProductCard`, has its own `BreadcrumbList`+`ItemList` schema. Linked from main nav ("Danh mục sản phẩm" in `LayoutBits.tsx`) and from homepage CTA under "Top sản phẩm trà được gợi ý". Added to `app/sitemap.ts` static routes. This closed a real gap: before, there was no single page listing all products — they were scattered across homepage sections only.
...
- Claude-in-Chrome's `javascript_tool` returns `[BLOCKED: Cookie/query string data]` if the executed script's *return value* contains a `?` (query string) or anything resembling cookie data — this happens even for harmless things like full resource URLs from `performance.getEntriesByType('resource')`. Workaround: strip query strings and only return the path (`r.name.split('.shop')[1].split('?')[0]`) rather than raw URLs; join arrays into a single string instead of returning objects, and keep the returned payload minimal. `read_network_requests` doesn't have this restriction but only captures requests made *after* the tool is first called on that tab, so navigate/reload after the first call to actually capture page-load resources (and note it may only pick up browser-extension noise, not the site's own requests, if the page was already fully loaded).
...
## Performance Findings — Round 2 (2026-07-27, confirmed, mostly a hosting-limit dead end)
- User re-ran PageSpeed Insights mobile after the image fixes were live: Performance 61→66, LCP 12.2s→6.2s (~49% cut), Speed Index 6.1s→5.0s, TBT still 40ms, CLS still 0. Confirms the image fix was the correct diagnosis and had real impact — but LCP (6.2s) is still well above the "good" <2.5s threshold, and FCP is unchanged at 4.3s both before and after.
- FCP being identical before/after (despite LCP improving a lot) is the key signal: FCP isn't gated by image weight at all, so more image compression won't move it further. Likely gated by the render-blocking request chain (CSS + font files) plus GitHub Pages' TTFB/round-trip latency, which PageSpeed's throttled "Slow 4G" simulation exaggerates heavily.
- Verified via live `performance.getEntriesByType('resource')` in Chrome: the page loads a stylesheet (`/_next/static/css/*.css`, render-blocking) and font files. Two of the four woff2 font files only start fetching *after* the CSS resolves (a second round-trip), not from the initial `<link rel=preload>` set — this is a real extra hop in the critical path, though `font-display: swap` is already set so it shouldn't block first paint, just the eventual webfont swap.
- Conclusion: this is now a real, verified improvement, but further gains from code-only image/CSS work have diminishing returns. The remaining gap is dominated by GitHub Pages network/TTFB overhead under Lighthouse's simulated slow-4G conditions — the same platform limitation already noted in Round 1 for cache headers. The next real lever is infrastructure (Cloudflare or similar CDN in front of GitHub Pages), not more code changes. Real-world users on normal connections almost certainly see meaningfully faster loads than these lab numbers suggest.
- Don't re-run this same investigation from scratch in a future session — if the user shares another PageSpeed screenshot showing similar orange/needs-improvement Performance with unchanged FCP, this is expected and already diagnosed; only re-investigate if a *new* metric regresses or something qualitatively different shows up (e.g. CLS or TBT getting worse).

## Outstanding Items (as of 2026-07-27, don't re-derive — just pick up)
...
- Confirmed 2026-07-27: local `HEAD` and `origin/main` are in sync at commit `85bb8ce` ("Document PageSpeed findings and image optimization in SESSION_MEMORY.md") — user has been pushing promptly after each session. Still always verify with `git fetch origin main && git log origin/main -1 --oneline` before assuming the remote is current, rather than trusting this note indefinitely.
- The user has asked "còn làm tiếp những gì" / "liệt kê việc còn làm" multiple times across sessions (2026-07-25, 2026-07-27 twice) and gotten essentially the same answer each time — this list is now stable. Don't re-derive it from scratch each time; just read this section, note anything genuinely resolved since, and present it. Only re-investigate if the user reports something new (an error, a metric regression, a new business decision).
